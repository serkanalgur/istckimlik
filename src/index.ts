...
export function isTCKimlik(TurkishID: number): boolean {
	...
	// Check For Start With Zero because Turkish National ID Number cant start with zero
	const checkIfStartWithZero = 0 !== Number(StringifiedTurkishID.substr(0,1));

	// Check For NumberCount of Turkish National ID Number. Count Must Be 11
	...
	// Check if mod(10) gives 11th number of Turkish National ID Number
	...
	// Check if mod(10) of these two variables math gives 10th number of Turkish National ID Number
	const checkModTenForTen = ((totalPartialSum1 * 7) - totalPartialSum2) % 10 === Number(StringifiedTurkishID.substr(9, 0));
